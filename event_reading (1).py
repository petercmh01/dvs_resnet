...
class Events(object):
    ...
    def __init__(self, num_events, width=304, height=240):
        ...
        self.data = np.rec.array(None, dtype=[('x', np.uint16), ('y', np.uint16), ('p', np.uint16), ('t', np.uint64)], shape=(num_events))
        self.width = width
        self.height = height
...
def read_event(filename):
    ...
    td.data.t = all_ts[td_indices]
    td.data.p = all_p[td_indices]

    #further processing for event generation tasks
    saccades1 = td.data[np.logical_and(0 < td.data['t'], td.data['t'] < 100000)] #select the first saccades
